...
// --- KONFIGURASI ADMIN ---
const ADMIN_A = "6289507913948"; // Nomor Kamu
            ...
            kalkulatorCont.innerHTML = `
                <div style="display: flex; gap: 10px; align-items: flex-end;">
                    <div style="flex: 1.5; text-align: left;">
                        ...
                        <input type="number" id="custom-qty" placeholder="Min: ${data.min}" style="width: 100%; padding: 12px; border-radius: 12px; border: 1px solid #ff85b3; background: #fff; margin-top: 5px; height:50px;" oninput="hitungHargaOtomatis(this.value, '${id}')">
                    </div>
                    <div style="flex: 1; text-align: left;">
                        <label style="font-size: 10px; color: #ff85b3; font-weight: bold; margin-left: 5px;">HARGA</label>
                        <div id="display-harga-otomatis" style="height: 50px; background: #fff; border-radius: 12px; border: 1px solid #ff85b3; display: flex; align-items: center; justify-content: center; font-weight: bold; color: #ff85b3; margin-top: 5px; font-size: 14px;">Rp0</div>
                    </div>
                </div>`;
        }
    }
...
function handleDeteksiOperator(nomor) {
    ...
    if (provider) {
        ...
        if (isTutup) {
            ...
            grid.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 25px 15px; border: 2px solid #ff85b3; border-radius: 20px; background: rgba(255,255,255,0.4);"><p style="font-size: 35px; margin-bottom: 10px;">🕒</p><h3 style="color: #ff85b3; margin-bottom: 5px;">LAYANAN PULSA TUTUP</h3><p style="font-size: 12px; color: #444; line-height: 1.5;">Mohon maaf, layanan pengisian pulsa hanya beroperasi pada pukul <b>04:00 - 22:00 WIB</b>.</p></div>`;
            return;
        }
...
function renderProducts(id) {
    ...
    if(data) {
        data.forEach(p => {
            ...
            grid.innerHTML += `<div onclick="selectItem('${p.item}', '${p.harga}', this)" class="product-card ${p.isPremium ? 'premium' : ''}"><span class="item-name" style="color:#000 !important;">${p.item} ${labelHTML}</span><span class="item-price">${p.harga}</span></div>`;
        });
    }
}
...
function renderProductsPulsa(provider) {
    ...
    if(pricelistPPOB[provider]) {
        pricelistPPOB[provider].forEach(p => {
            ...
            grid.innerHTML += `<div onclick="selectItem('${p.item}', '${p.harga}', this)" class="product-card"><span class="item-name" style="color:#000 !important;">${p.item} ${labelHTML}</span><span class="item-price">${p.harga}</span></div>`;
        });
    }
}
...
function tampilkanKonfirmasi() {
    ...
    if (currentServiceId === 'pulsa' && val.length < 10) return kustomAlert("Nomor Tidak Valid", "Nomor HP minimal harus 10 digit!", "📱");
...
function kirimTestiWA() {
    ...
    const pesanTesti = `*TESTIMONI SYRUMI STORE [${admin.label}]*%0A%0A"${t}"%0A%0A_Terima kasih sudah belanja di Syrumi Store!_`;
...
function hitungHargaOtomatis(qty, id) {
    ...
    const data = hargaSatuan[id];
    document.querySelectorAll('.product-card').forEach(c => c.classList.remove('selected'));
    if (!qty || qty <= 0) { display.innerText = "Rp0"; selectedProduct = ""; selectedPrice = ""; return; }
    if (qty < data.min) { display.innerHTML = `<span style="color:#ff4d4d; font-size:10px;">Min: ${data.min.toLocaleString()}</span>`; selectedProduct = ""; selectedPrice = ""; return; } 
    if (qty > data.max) { display.innerHTML = `<span style="color:#ff4d4d; font-size:10px;">Limit!</span>`; selectedProduct = ""; selectedPrice = ""; return; }
